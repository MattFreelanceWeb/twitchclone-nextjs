...
function Sidebar({}: Props) {
  ...
  return (
    <div className={`fixed w-60 bg-[#19171c] h-screen flex flex-col items-center duration-500 ${toggle?'-translate-x-60' :'translate-x-0'} z-40 `} >
        ...
        <button className={` translate-x-36 ${toggle? 'flex' : 'hidden'}`} onClick={()=>{setToggle(!toggle)}}><span className='text-black '><IonIcon name='chevron-forward' size='large'/></span> </button>
    </div>
  )
}
